...
async function main() {
  ...
  const wrappedFCashFactory = IWrappedfCashFactory__factory.connect(
  ...
  const lowestYieldfCash = IWrappedfCashComplete__factory.connect(
    await wrappedFCashFactory.computeAddress(3, lowestYieldMarket.maturity),
    account
  )
  const highestYieldfCash = IWrappedfCashComplete__factory.connect(
    await wrappedFCashFactory.computeAddress(3, highestYieldMarket.maturity),
    account
  )
