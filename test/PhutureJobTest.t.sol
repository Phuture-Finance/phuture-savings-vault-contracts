...
contract PhutureJobTest is Test {
    ...
    function testCannotHarvest() public {
        ...
        vm.expectRevert(bytes("AccessControl: account 0x0b7108e278c2e77e4e4f5c93d9e5e9a11ac837fc is missing role 0x3fc733b4d20d27a28452ddf0e9351aced28242fe03389a653cdb783955316b9b"));
        phutureJob.harvest(FRPVaultProxy);

        // cannot harvest if TIMEOUT
        FRPVaultProxy.grantRole(keccak256("HARVESTER_ROLE"), address(phutureJob));
        phutureJob.harvest(FRPVaultProxy);
        vm.expectRevert(bytes("PHUTURE_JOB:TIMEOUT"));
        phutureJob.harvest(FRPVaultProxy);
    }

    function testSlippageScaledAmount() public {
        jobConfig.setHarvestingAmountSpecification(jobConfig.getHarvestingSpecification(3));
...
//        assertEq(FRPVaultProxy.lastHarvest(), block.timestamp);
        ...
        // scales and harvests
        FRPVaultProxy.deposit(5_000_000 * 1e6, usdcWhale);
        phutureJob.harvest(FRPVaultProxy);
    }
