...
contract FrpVaultTest is Test {
    ...
    function testWithdrawalFromBothMaturities() public {
        ...
        assertEq(highestYieldFCash.balanceOf(address(FRPVaultProxy)), 6467496096);
        assertEq(FRPVaultProxy.balanceOf(usdcWhale), 0);
        assertEq(usdc.balanceOf(address(FRPVaultProxy)), 996);

        uint balanceAfterWithdrawal = usdc.balanceOf(usdcWhale);
        // User losses certain amount of USDC due to slippage
        assertEq(balanceBeforeDeposit - balanceAfterWithdrawal, 101560598);
    }
    ...
    // TODO vm.expectRevert has a bug. should be named testCannotUpgradeWithoutRole()
